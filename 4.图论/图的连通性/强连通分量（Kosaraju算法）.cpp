...
//第一次dfs得到的数组是块状的，也就是每个强连通分量的节点在一起，接下来才是其他强连通分量
//dfs的时候会遍历到其他的强连通分量，但是由于是最后才放进数组，所以会把一个强连通分支走完
//每个块进行缩点，数组内得到的是逆拓扑序
//第二次dfs从一个强连通分量不会走到另一个强连通分量
//若能走到说明反图中u（第一个强连通分量）->v（第二个强连通分量）存在，即原图中v->u存在
//逆拓扑序说明原图中有u->v，这是两个强连通分支（矛盾）